...
const fetchImageAsBase64 = async (url) => {
  ...
  export const generateFIRCopy = async (rawCaseItem) => {
    ...
    // Check if individualdetails exists before proceeding
    const individualDetails =
      caseItem.individualdetails &&
      typeof caseItem.individualdetails === "string"
        ? JSON.parse(caseItem.individualdetails)
    ...
    // Prevent further execution if individualDetails is null or undefined
    if (!individualDetails) {
      console.error("Error: individualdetails is null or undefined");
      return; // Exit the function early if individualDetails is not available
    }
    ...
    // Seal - Add a circular seal at the bottom left
